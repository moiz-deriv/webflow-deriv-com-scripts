window.establishWebSocketConnection = () => {
    return new Promise((resolve, reject) => {
        ...
        const langCookie = window.parseCookies(document.cookie, "webflow-user-language");
        const language = langCookie ? langCookie.toLowerCase() : 'en';
        const server_url = localStorage.getItem('config.server_url') || 'green.derivws.com';
        window.websocket = new WebSocket(`wss://${server_url}/websockets/v3?app_id=${getAppId()}&l=${language || 'en'}&brand=deriv`);
